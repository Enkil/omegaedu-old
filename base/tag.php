...
$activepage='base'; include('../../header.php');

$tagname = '';
if($tag=='math') $tagname='Математика';
if($tag=='phys') $tagname='Физика';
if($tag=='rus') $tagname='Русский язык';
if($tag=='egenews') $tagname='Новости ЕГЭ';
if($tag=='vuz') $tagname='О вузах';
if($tag=='interview') $tagname='Интервью';
...
<p style="font-size:24px; margin-bottom:20px;"><a class="black" href="/base">База знаний</a> / <? echo($tagname); ?></p>


<div style="display:inline-block; width:400px; vertical-align:top;"><a href="#"><img src="http://www.examen.ru/assets/images/Articles/hazy_fizteh_00.JPG" style="width:400px;" /></a></div>
<div style="display:inline-block; width:455px; margin-left:15px;">
<p style="font-size:21px; margin-top:5px;"><a class="black" href="#">«Система Физтеха». Все о лучшем техническом вузе страны.</a></p>
<p>
...
<p style="color:#777; font-size:14px; margin-bottom:0px;"><i style="font-size:15px;" class="fa fa-eye"></i> <span id="views">1274</span><br>11 января 2014</p>
</div>

<hr style="border-top:1px solid #ccc;">

<div style="display:inline-block; width:400px; vertical-align:top;"><img src="http://www.examen.ru/assets/images/Articles/hazy_fizteh_00.JPG" style="width:400px;" /></div>
<div style="display:inline-block; width:455px; margin-left:15px;">
<p style="font-size:21px; margin-top:5px;">«Система Физтеха». Все о лучшем техническом вузе страны.</p>
<p>
МФТИ (Московский физико-технический институт, он же Долгопрудненский физкультурный техникум с английским уклоном, он же чаще всего подразумевается под словом Физтех) — бывший заповедник матана, гиков, задротов и прочих ебанутых. Разумеется, уже не тот, как и вообще всё образование в этой стране, но куны ещё встречаются, хотя сабж чуть более, чем наполовину наполнен мечтающими свалить в этот ваш менеджмент долбоёбами.
</p>
<p><a href="#">Читать полностью..</a></p>
...
$acol = mysql_result($r, 0);
$r = mysql_query("SELECT MAX(id) FROM video WHERE 1=1");
$vcol = mysql_result($r, 0);
$col = $acol + $vcol;

for($i = 0; $i < $acol; $i++)
{
  $r = mysql_query("SELECT * FROM article WHERE id = '".($acol-$i)."' AND theme='".$tag."'");
...
for($i = 0; $i < $vcol; $i++)
{
  $r = mysql_query("SELECT * FROM video WHERE id = '".($vcol-$i)."' AND theme='".$tag."'");
  if(mysql_num_rows($r)!=0)
  {
    $data[$i+$acol] = mysql_fetch_array($r);
    $data[$i+$acol]['type'] = "video";
  }
  else $col--;
}
