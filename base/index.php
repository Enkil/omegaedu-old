...
<p style="font-size:24px; margin-bottom:20px;">База знаний</p>
...
{
<div style=" border:2px solid #ccc; border-radius:15px; padding:20px 30px 20px 30px; margin-top:20px;">
<p style="font-size:24px; margin-bottom:20px;">Все статьи</p>
...
<p style="color:#777; font-size:14px;"><i style="font-size:15px;" class="fa fa-eye"></i> <span id="views">1274</span><br>11 января 2014</p>
  ...
  $r = mysql_query("SELECT * FROM article WHERE id = '".($acol-$i)."'");
  $data[$i] = mysql_fetch_array($r);
  $data[$i]['type'] = "article";
}
for($i = 0; $i < $vcol; $i++)
{
  $r = mysql_query("SELECT * FROM video WHERE id = '".($vcol-$i)."'");
